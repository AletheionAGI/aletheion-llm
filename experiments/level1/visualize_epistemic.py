...
def plot_q1_q2_scatter(
    ...
    scatter = ax.scatter(all_Q1, all_Q2, c=all_heights, cmap='viridis',
...
def plot_uncertainty_vs_error(
    ...
    for i in range(n_bins):
        bin_mask = (all_uncertainty >= bin_boundaries[i]) & (all_uncertainty < bin_boundaries[i + 1])
        if bin_mask.sum() > 0:
    ...
    ax.errorbar(bin_centers, mean_errors, yerr=std_errors, fmt='o-',
               label='Mean error ± std', capsize=5, linewidth=2)
...
def main():
    ...
    parser.add_argument('--examples', type=str, default=None,
                       help='Path to examples file (one per line)')
    parser.add_argument('--output', type=str, default='outputs/epistemic_viz',
    ...
    for ex in analyzed_examples:
        json_ex = {k: v.tolist() if isinstance(v, np.ndarray) else v
                  for k, v in ex.items()}
