...
def evaluate_baseline(
    model: GPT2LMHeadModel, loader: DataLoader, device: torch.device, max_batches: int = 100
) -> dict:
...
def plot_calibration_comparison(baseline_results: dict, pyramidal_results: dict, save_path: Path):
    ...
    for idx, (results, name) in enumerate(
        ...
        ax.plot(
            ...
            accuracies,
            "o-",
            label=f'{name} (ECE={results["ece"]:.4f})',
            linewidth=2,
        )
...
def plot_perplexity_ece_scatter(baseline_results: dict, pyramidal_results: dict, save_path: Path):
...
def statistical_significance_test(baseline_results: dict, pyramidal_results: dict) -> dict:
    ...
    t_stat, p_value = stats.ttest_ind(
        baseline_results["correctness"], pyramidal_results["correctness"]
    )
...
def create_summary_report(
    baseline_results: dict, pyramidal_results: dict, stat_results: dict, save_path: Path
):
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=2, help="Batch size (default: 2, reduce if OOM)"
    )
    ...
    plot_calibration_comparison(
        baseline_results, pyramidal_results, output_dir / "calibration_plots.png"
    )

    # Plot perplexity vs ECE
    plot_perplexity_ece_scatter(
        baseline_results, pyramidal_results, output_dir / "perplexity_ece_scatter.png"
    )

    # Create summary report
    create_summary_report(
        baseline_results, pyramidal_results, stat_results, output_dir / "report.md"
    )
