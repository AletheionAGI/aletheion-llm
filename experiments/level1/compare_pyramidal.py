...
def train_pyramidal(
    ...
    while step < steps:
        ...
        history["w_memory"].append(
            outputs.pyramid["w_memory"][..., :-1, :][valid_mask].mean().item()
        )
        history["w_pain"].append(outputs.pyramid["w_pain"][..., :-1, :][valid_mask].mean().item())
        history["w_choice"].append(
            outputs.pyramid["w_choice"][..., :-1, :][valid_mask].mean().item()
        )
        history["w_exploration"].append(
            outputs.pyramid["w_exploration"][..., :-1, :][valid_mask].mean().item()
        )

        pbar.set_postfix(
            {"loss": f"{loss.item():.4f}", "height": f"{loss_dict['mean_height']:.3f}"}
        )
        pbar.update(1)
        step += 1
...
def plot_comparison(baseline_hist: dict, pyramidal_hist: dict, save_dir: Path):
    ...
    if baseline_hist["eval_perplexity"] and pyramidal_hist["eval_perplexity"]:
        steps_baseline = np.linspace(
            0, len(baseline_hist["train_loss"]), len(baseline_hist["eval_perplexity"])
        )
        steps_pyramidal = np.linspace(
            0, len(pyramidal_hist["train_loss"]), len(pyramidal_hist["eval_perplexity"])
        )
        axes[0, 1].plot(
            steps_baseline, baseline_hist["eval_perplexity"], label="Baseline", marker="o"
        )
        axes[0, 1].plot(
            steps_pyramidal, pyramidal_hist["eval_perplexity"], label="Pyramidal", marker="s"
        )
    ...
    axes[0, 2].plot(
        pyramidal_hist["target_height"], label="Target Height", color="green", alpha=0.7
    )
    ...
    # Performance summary
    final_baseline_ppl = (
        baseline_hist["eval_perplexity"][-1] if baseline_hist["eval_perplexity"] else 0
    )
    final_pyramidal_ppl = (
        pyramidal_hist["eval_perplexity"][-1] if pyramidal_hist["eval_perplexity"] else 0
    )
    improvement = (
        ((final_baseline_ppl - final_pyramidal_ppl) / final_baseline_ppl * 100)
        if final_baseline_ppl > 0
        else 0
    )
    ...
    axes[1, 2].text(
        0.1, 0.2, f"Final Base Stability: {pyramidal_hist['base_stability'][-1]:.3f}", fontsize=11
    )
    axes[1, 2].axis("off")
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="outputs/comparison_pyramidal", help="Output directory"
    )
    ...
    baseline_hist = train_baseline(
        baseline_model, train_loader, val_loader, device, args.steps, args.eval_interval, args.lr
    )
    ...
    pyramidal_hist = train_pyramidal(
        pyramidal_model,
        train_loader,
        val_loader,
        device,
