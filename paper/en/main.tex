...
\caption*{(b) Pyramidal Architecture}
\end{minipage}
\caption{\textbf{Training curves comparison.} 

Baseline (Fig. 4):
"Standard GPT-2 training showing typical convergence behavior. ECE remains low (~0.01-0.02) throughout training as the model learns without epistemic constraints. Serves as control for comparing architectural interventions."
Pyramidal (Fig. 5):
"Pyramidal architecture demonstrating the Skynet phenomenon. Height progresses from 0.624→1.000 (apex delusion at 60k steps) while ECE degrades from 0.011→0.084 (7.6× worse). Epistemic Uncertainty collapses to near-zero as the model becomes overconfident. This documents what happens without explicit epistemic gates: the system drifts toward delusional omniscience."
Key contrast: Baseline = normal learning. Pyramidal = architectural collapse into overconfidence.
