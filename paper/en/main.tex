...
\end{subfigure}
\caption{\textbf{Comparative training dynamics: Baseline vs. Pyramidal architectures over 60,000 steps.} \textbf{Left panel (Baseline):} Standard transformer showing \textcolor{blue}{training loss} (solid blue) and \textcolor{red}{validation loss} (dashed red) decreasing monotonically, with \textcolor{green}{perplexity} (green) remaining stable. The baseline exhibits traditional learning patterns without epistemic awareness. \textbf{Right panel (Pyramidal):} Ungated pyramidal architecture showing similar convergence patterns but with the architectural foundation for epistemic quantification. Both architectures demonstrate stable training, but neither exhibits the \textbf{Skynet Phenomena}---the emergent metalearning behavior that only appears when Q1/Q2 epistemic gates are activated (Figure~\ref{fig:pyramidal_q1q2_curves}). The pyramidal architecture provides the geometric structure necessary for uncertainty quantification, while maintaining comparable training stability to the baseline. Performance metrics show \textcolor{blue}{training curves} converging smoothly alongside \textcolor{red}{validation curves}, with \textcolor{green}{perplexity} tracking model confidence. This establishes the baseline for comparison with the fully gated Q1/Q2 system that follows.}
